...
void LoadPanel::LoadPreview(const string &identifier)
{
	loadedInfo.Clear();
	if(identifier.starts_with("commit:"))
	{
		string commitHash = identifier.substr(7, 40);
		filesystem::path repoPath = Files::Saves() / selectedPilot;
		...
		if (!content.empty()) {
			loadedInfo.Load(content);
		}
	}
	else
	{
		loadedInfo.Load(Files::Saves() / identifier);
	}
}
